...
const update = async (req, res, next) => {
  ...
  if(status === 'cancelled') {
    ...
    return next({ status: 400, message: `Can not update '${currentStatus}' reservations.` });
  }
...
const updateReservation = async (req, res, next) => {
  ...
  res.json({ data: await service.updateReservationData(knex, res.locals.reservation) });
}
...
const validateReservation = (req, res, next) => {
  ...
  const hasProperties = (req, res, next) => {
    ...
    if(first_name.length <= 0) {
      return next({ status: 400, message: `first_name needs a length greater than or equal to 1` });
    }

    if(last_name.length <= 0) {
      return next({ status: 400, message: `last_name needs a length greater than or equal to 1` });
    }

    if(mobile_number.length <= 0) {
      return next({ status: 400, message: `mobile_number needs a length greater than or equal to 1` });
    }

    if(reservation_date.length <= 0) {
      return next({ status: 400, message: `reservation_date needs a length greater than to equal to 1` });
    }
    ...
    if(reservation_time.length <= 0) {
      return next({ status: 400, message: `reservation_time needs a length greater than or equal to 1` });
    }
    ...
    if(reservation_time < "10:30" || reservation_time > "21:30") {
      return next({ status: 400, message: "Reservations can't be placed when closed. Hours: 10:30AM - 09:30PM" });
    }

    if(typeof people !== 'number' || people <= 0) {
      return next({ status: 400, message: `people needs to be greater then or equal to 1` });
    }
    ...
    if(new Date(reservation_date).valueOf() < date.valueOf() 
      && date.toUTCString().slice(0, 16) !== new Date(reservation_date).toUTCString().slice(0, 16)) {
      return next({ status: 400, message: `reservation_date must be in the future` });
    }
