...
const update = async (req, res, next) => {
  ...
  const reservation = await reservationService.getReservation(knex, reservation_id);
  ...
  if(table.reservation_id !== null) {
    return next({ status: 400, message: `${table.table_name} is currently occupied by ${reservation.reservation_id}` });
  }

  if(table.capacity < reservation.people) {
    return next({ status: 400, message: `${table.table_name} does not have a capacity to seat ${reservation.people} people.` });
  }
...
const unreserve = async (req, res, next) => {
  ...
  if(reservation_id === null) {
    return next({ status: 400, message: `'${table_name}' is currently not occupied.` });
  }
...
const hasProperties = (req, res, next) => {
  ...
  if(!table_name || table_name.length <= 1) {
    return next({ status: 400, message: `"table_name" key is required and must have a length greater than 1.` });
  }

  if(!capacity || typeof capacity !== "number" || capacity === 0) {
    return next({ status: 400, message: `"capacity" key is required and must be a integer greater than or equal to 1.` });
  }
